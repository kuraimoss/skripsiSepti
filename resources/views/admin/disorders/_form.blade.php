...
<form method="POST" action="{{ $action }}" class="space-y-6 rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
    ...
    <div class="flex flex-col-reverse gap-3 border-t border-slate-200 pt-5 sm:flex-row sm:justify-end">
        <a href="{{ \Illuminate\Support\Facades\Route::has('admin.disorders.index') ? route('admin.disorders.index') : '#' }}" class="inline-flex items-center justify-center gap-2 rounded-md border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-100">
