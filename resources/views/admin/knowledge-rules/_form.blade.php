...
<form method="POST" action="{{ $action }}" class="space-y-6 rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
    ...
    <div class="flex flex-col-reverse gap-3 border-t border-slate-200 pt-5 sm:flex-row sm:justify-end">
        <a href="{{ \Illuminate\Support\Facades\Route::has('admin.knowledge-rules.index') ? route('admin.knowledge-rules.index') : '#' }}" class="inline-flex justify-center rounded-md border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-100">Batal</a>
        <button type="submit" class="inline-flex justify-center rounded-md bg-teal-700 px-4 py-2 text-sm font-semibold text-white transition hover:bg-teal-800">Simpan Aturan</button>
    </div>
</form>
